...
# - Modular design with separate concerns (UI, API, ML, Data)
# - Session state for user data persistence across interactions
# - ML-based recommendation system trained on user preferences
# - Real-time API integration with Fragella perfume database
...
# Make sure the API key exists before we do anything else
...
USER_INTERACTIONS_FILE = "data/user_interactions.json"  # Tracks what users click, view, favorite
USER_INVENTORY_FILE = "data/user_perfume_inventory.json"  # User's perfume collection
PERFUME_RANKINGS_FILE = "data/perfume_rankings.json"  # Popularity scores for each perfume
ML_MODEL_FILE = "data/ml_model.pkl"  # Trained ML model
ML_SCALER_FILE = "data/ml_scaler.pkl"  # Feature scaler for the ML model

ML_CONFIG = {
    # ML training configuration - balanced to avoid overfitting
    'negative_samples_ratio': 2,  # For each perfume they own, sample 2 they don't (for training)
    'min_inventory_size': 2,  # Need at least 2 perfumes before we can train the model
    'model_type': 'logistic_regression',  # Using logistic regression (simple and works well)
    'random_state': 42,  # For reproducibility
    'min_recommendation_probability': 0.5,  # Only show recommendations with >50% confidence
    'diversity_threshold': 0.3  # Makes sure we don't recommend 10 similar perfumes
}

# Logo configuration for the header
# You can use either a text logo (default) or upload your own PNG image
...
def get_image_base64(image_path: str) -> str:
    """Converts an image to base64 so we can embed it in HTML"""
    import base64
    with open(image_path, "rb") as img_file:
        return base64.b64encode(img_file.read()).decode()


# Functions for loading and saving user data (now using imported modules)
# Wrappers to maintain compatibility with existing code

def load_user_interactions() -> List[Dict]:
    """Loads user interaction history - now using modular function"""
    return _load_interactions(USER_INTERACTIONS_FILE)

def save_user_interactions(interactions: List[Dict]):
    """Saves user interaction history - now using modular function"""
    _save_interactions(interactions, USER_INTERACTIONS_FILE)

def load_user_inventory() -> List[Dict]:
    """Loads user perfume collection - now using modular function"""
    return _load_inventory(USER_INVENTORY_FILE)

def save_user_inventory(inventory: List[Dict]):
    """Saves user perfume collection - now using modular function"""
    _save_inventory(inventory, USER_INVENTORY_FILE)

def load_perfume_rankings() -> Dict:
    """Loads perfume popularity rankings - now using modular function"""
    return _load_rankings(PERFUME_RANKINGS_FILE)

def save_perfume_rankings(rankings: Dict):
    """Saves perfume popularity rankings - now using modular function"""
    _save_rankings(rankings, PERFUME_RANKINGS_FILE)



# ===== API FUNCTIONS NOW IN api/fragella.py =====
# call_fragella_api, search_fragella_perfumes, transform_api_perfume, get_initial_perfumes

# ===== ML FUNCTIONS NOW IN ml/recommender.py =====
# extract_perfume_features, train_ml_model, get_ml_recommendations, etc.

# ===== DATA PERSISTENCE WRAPPERS =====
# Wrapper functions to maintain compatibility with existing code

def load_user_interactions() -> List[Dict]:
    """Loads user interaction history"""
    return _load_interactions(USER_INTERACTIONS_FILE)
...
def load_user_inventory() -> List[Dict]:
    """Loads user perfume collection"""
    return _load_inventory(USER_INVENTORY_FILE)

def save_user_inventory(inventory: List[Dict]):
    """Saves user perfume collection"""
    _save_inventory(inventory, USER_INVENTORY_FILE)

def load_perfume_rankings() -> Dict:
    """Loads perfume popularity rankings"""
    return _load_rankings(PERFUME_RANKINGS_FILE)

def save_perfume_rankings(rankings: Dict):
    """Saves perfume popularity rankings"""
    _save_rankings(rankings, PERFUME_RANKINGS_FILE)
...
def record_interaction(perfume_id: str, interaction_type: str):
    """Track user interactions with perfumes"""
...
def update_perfume_rankings():
    """Update perfume popularity rankings"""
    # Load all interactions
    interactions = load_user_interactions()
    
    rankings = {}
    for interaction in interactions:
        # Only count clicks for popularity
...
def get_ml_sorted_perfumes(perfumes: List[Dict]) -> List[Dict]:
    """Sort perfumes by ML scores"""
    # Use ML model predictions if available, fallback to popularity ranking
    """Sort perfumes by ML scoresPerfumes with more clicks appear first (popularity based on clicks only).
...
# Session state and app styling

def initialize_session_state():
    """
    Sets up all the session variables when the app first loads.
    Streamlit reruns everything on every interaction, so we need this to remember stuff.
    """
...
# apply_custom_styling() is imported from ui.styles

# Header and UI components

def render_header():
    """
    Renders the header with the Scentify logo at the top.
    Clicking it takes you back to the home page.
    """
    # Create centered logo and tagline
    if LOGO_TYPE == "image":
...
def display_perfume_card(perfume: Dict, show_ml_badge: bool = False, source: str = 'search', card_index: int = None):
    """
    Display a modern, beautiful perfume card with creative design.
    
    """
    # Create unique key suffix to avoid duplicate widget IDs
    if card_index is not None:
        key_suffix = f"{source}_{card_index}_{perfume['id']}"
    else:
        # Use a counter from session state to ensure uniqueness
...
def display_inventory_perfume_card(perfume: Dict, index: int):
    """
    Display a perfume card in the inventory with the same design as search results.
...
def create_donut_chart(note_counter: Counter, title: str):
    ...
    Shows top 10 notes and groups the rest as 'Rest'.
...
def add_to_user_inventory(perfume: Dict) -> bool:
    """Add perfume to user inventory with duplicate prevention"""
    # Returns True if added, False if already exists
    """
    Add a perfume to user's inventory.
    Unlimited perfumes allowed!
    Returns True if added, False if already exists.
    """
...
def render_landing_page():
    """
    The main landing page with three options:
    1. Search - find specific perfumes
    2. Questionnaire - answer questions to get recommendations
    3. Your Collection - save and manage your perfumes (gets ML recommendations)
    """
    # Add invisible anchor at absolute top to make sure scroll position
...
def render_user_favorites():
    """
    Display top 3 most popular perfumes based on clicks.
    """
    rankings = load_perfume_rankings()
...
def render_search_section():
    """
    The search page - lets users search for perfumes and filter by various criteria.
    """
    # Add invisible anchor at absolute top to make sure scroll position
    st.markdown('<div id="page-top" style="height: 1px;"></div>', unsafe_allow_html=True)
    ...
    # Complete list of ALL perfume brands (A-Z) - no duplicates, over 100 brands
...
def render_questionnaire_section():
    """
    The questionnaire page - users answer questions about their preferences.
    sweet, light vs. strong, etc.
    """
    # Add invisible anchor at absolute top to make sure scroll position
    st.markdown('<div id="page-top" style="height: 1px;"></div>', unsafe_allow_html=True)
...
def get_questionnaire_recommendations() -> List[Dict]:
    ...
    Uses a scoring system to match user profile with perfume attributes.
...
def render_inventory_section():
    """
    Your Collection page - shows saved perfumes with stats and ML recommendations.
    Once you add 2+ perfumes, the ML kicks in and suggests new ones.
    """
    # Add invisible anchor at absolute top to make sure scroll position
    st.markdown('<div id="page-top" style="height: 1px;"></div>', unsafe_allow_html=True)
    ...
    cols_per_row = 3  # 3 columns for better responsive layout
                ...
                with cols[j]:
                    if all_items[idx] == 'add':
                        ...
                        if st.button("Add New", key="btn_add_perfume", use_container_width=True, type="primary"):
                            # Go directly to search section instead of separate add view
                            st.session_state.active_section = "search"
                            st.rerun()
                    else:
    ...
    <div style="flex: 1; padding: 14px 16px 16px 16px; display: flex; flex-direction: column; align-items: center; justify-content: space-between;">
        ...
        <div style="width: 100%; background: linear-gradient(135deg, #f8f7fa 0%, #ffffff 100%); padding: 10px; border-radius: 12px; border: 1px solid #e8e4f0; min-height: 65px; display: flex; flex-direction: column; justify-content: center;">
            ...
            <p style="text-align: center; color: #6b5b95; font-size: 12px; font-weight: 600; margin: 0; line-height: 1.3; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">{accords_str}</p>
                    ...
                    # Add spacing between card and buttons
                    st.markdown('<div style="margin-top: 25px; margin-bottom: 10px;"></div>', unsafe_allow_html=True)
                    
                    # Action buttons (View and Add only)
...
def main():
    ...
    if st.session_state.active_section == "home":
        ...
        render_inventory_section()  # Your saved perfume collection
