...
def search_fragella_perfumes(api_key: str, query: str, limit: int = 20) -> List[Dict]:
    """
    Searches for perfumes matching what the user typed.
    Needs at least 3 characters to search (otherwise we'd return thousands of results).
    Returns up to 20 perfumes by default.
    """
...
def transform_api_perfume(api_perfume: Dict) -> Dict:
    """
    Converts the API's perfume format to our own format.
    The Fragella API has a weird structure, so we clean it up and normalize everything.
    """
    ...
    # Parse seasonality from Season Ranking
    seasonality = {}
    ...
    # Parse occasion from Occasion Ranking
    ...
    # Get perfume type from OilType field
    ...
    # Get price from API or use default
    price = api_perfume.get('price', api_perfume.get('Price', 75))
    try:
        price = int(price)
    except:
        price = 75  # Default price if parsing fails
...
def get_initial_perfumes(api_key: str) -> List[Dict]:
    ...
    Cached for 2 hours to prevent repeated API calls.
    """
    perfumes = []
    
    # Expanded list of popular perfume brands and scent terms
    search_terms = [
        # Luxury brands
